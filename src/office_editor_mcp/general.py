...
try:
    ...
    # 只在 DEBUG 级别输出，避免干扰 MCP stdio 通信
    import sys

    if "--debug" in sys.argv:
        logger.warning(f"部分功能依赖未安装: {e}")
        logger.info("可选依赖: pip install pdf2docx PyPDF2 pytesseract deepl")
...
def replace_placeholders(
    file_path: str, data_mapping: Dict[str, List[str]], index: int
):
    """替换文档中的占位符"""
                ...
                for row in table.rows:
                    for cell in row.cells:
                        for paragraph in cell.paragraphs:
                            for key, values in data_mapping.items():
                                placeholder = f"{{{key}}}"
                                if placeholder in paragraph.text and index < len(
                                    values
                                ):
                                    paragraph.text = paragraph.text.replace(
                                        placeholder, values[index]
...
# OCR识别功能
@mcp.tool()
def ocr_recognize_text(
    image_path: str, language: str = "chi_sim+eng"
) -> Dict[str, Any]:
    """
    从图片中提取文本内容
    ...
    try:
        ...
        output_file = os.path.join(
            OUTPUT_DIR, os.path.splitext(os.path.basename(image_path))[0] + "_ocr.txt"
        )
...
def compare_documents(
    doc1_path: str, doc2_path: str, output_format: str = "html"
) -> Dict[str, Any]:
    """
    比较两个文档的内容差异
    ...
    try:
        ...
        if output_format == "html":
            ...
            result = diff.make_file(
                lines1, lines2, os.path.basename(doc1_path), os.path.basename(doc2_path)
            )
...
def translate_document(
    doc_path: str, target_language: str = "ZH", api_key: str = None
) -> Dict[str, Any]:
    """
    将文档内容翻译成指定目标语言
    ...
    try:
        ...
        output_file = os.path.join(
            OUTPUT_DIR, f"{filename}_translated_{target_language}.txt"
        )
        with open(output_file, "w", encoding="utf-8") as f:
...
def encrypt_document(doc_path: str, password: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "message": "文档加密完成", "output_file": output_file}
...
def decrypt_document(
    encrypted_file_path: str, password: str, output_format: str = None
) -> Dict[str, Any]:
    """
    解密文档
    ...
    try:
        ...
        return {"success": True, "message": "文档解密完成", "output_file": output_file}
...
# 数据库数据导入到Excel功能
@mcp.tool()
def import_database_to_excel(
    db_connection_string: str, query: str, output_file: str = None
) -> Dict[str, Any]:
    """
    从数据库导入数据到Excel文件
...
def batch_create_documents(
    ...
    output_prefix: str,
    count: int,
    data_mapping: Dict[str, List[str]] = None,
) -> Dict[str, Any]:
    """
    基于模板批量生成文档
    ...
    try:
        ...
        if data_mapping:
            ...
            if min_data_count < count:
                return {
                    "success": False,
                    "message": f"数据不足，最少需要{count}条记录，但只有{min_data_count}条",
                }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    results.append(
                        {"success": False, "message": str(e), "file_path": file}
                    )
                    failed_files.append(file)
...
# 文件目录操作功能
@mcp.tool()
def general_file_operations(
    operation: str, source_path: str, target_path: str = None, recursive: bool = False
) -> Dict[str, Any]:
    """
    通用文件和目录操作
    ...
    try:
        ...
        if operation == "copy":
            ...
            if os.path.isdir(source_path):
                ...
                return {
                    "success": True,
                    "message": f"成功将{source_path}复制到{target_path}",
                }
            else:
                # 复制文件
                ...
                return {
                    "success": True,
                    "message": f"成功将{source_path}复制到{target_file}",
                }
            ...
            return {
                "success": True,
                "message": f"成功将{source_path}移动到{target_path}",
            }
