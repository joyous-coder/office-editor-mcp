...
def add_text_to_document(
    ...
    Args:
        ...
        text: 要添加的文本内容
        is_heading: 是否作为标题添加
        heading_level: 标题级别 (1-9)，仅当is_heading=True时有效
        alignment: 对齐方式，可选值: "left", "center", "right", "justify"
        paragraph_index: 段落索引 (从0开始计数)，指定添加内容的位置，默认为-1表示文档末尾
        direction: 指定内容添加的位置，可选值: "front"(段落前), "behind"(段落后)，默认为behind
    ...
    if is_heading and (heading_level < 1 or heading_level > 9):
        return "错误: 标题级别必须在1至9之间"

    # 校验direction参数
    if direction not in ["front", "behind"]:
        return f"错误: 无效的方向参数 '{direction}'，可选值为: front, behind"
    ...
    if alignment not in alignment_map:
        return f"错误: 不支持的对齐方式 '{alignment}'，可选值为: left, center, right, justify"
    ...
    try:
        ...
        if paragraph_index == -1:
            ...
            position_msg = (
                f"第 {paragraph_index + 1} 段落{'前' if direction == 'front' else '后'}"
            )

        return f"成功在{position_msg}添加了{'标题' if is_heading else '文本'}"
    except Exception as e:
        return f"向Word文档添加内容时出错: {str(e)}"
...
def format_text_in_document(
    ...
    if not docx_installed:
        return (
            "错误: 无法格式化Word文档，请先安装python-docx库: pip install python-docx"
        )
    ...
    try:
        ...
        if not paragraph.text.strip():
            return (
                f"警告: 段落 {paragraph_index + 1} 为空或只包含空白字符，无法设置格式"
            )
...
def set_paragraph_spacing(
    ...
    # 打印接收到的参数，用于调试
    print(
        f"接收到的参数: file_path={file_path}, paragraph_index={paragraph_index}, "
    ...
    try:
        ...
        if line_spacing is not None:
            # 设置行间距规则
            paragraph.paragraph_format.line_spacing_rule = spacing_rule_map[
                line_spacing_rule
            ]
...
def edit_table_cell(
    ...
    try:
        ...
        if table_index < 0 or table_index >= len(doc.tables):
            return (
                f"错误: 无效的表格索引 {table_index}，文档共有 {len(doc.tables)} 个表格"
            )
...
def save_document_as(
    file_path: str, output_format: str = "docx", new_filename: str = None
) -> str:
    """
    将Word文档保存为指定格式。
        ...
        if output_format.lower() == "pdf":
            ...
            try:
                ...
                if output_format.lower() == "docx":
                    ...
                    return (
                        f"成功将文档保存为 DOCX 格式: {os.path.basename(output_path)}"
                    )
                else:
...
def close_document(file_path: str, save_changes: bool = True) -> str:
    ...
    try:
        ...
        try:
            ...
            return f"成功关闭文档: {os.path.basename(file_path)}" + (
                " 并保存更改" if save_changes else ""
            )
...
def edit_paragraph_in_document(
    file_path: str, paragraph_index: int, new_text: str, save: bool = True
) -> str:
    """
    编辑Word文档中指定段落的文本内容。
...
def find_and_replace_text(
    ...
    try:
                    ...
                    for paragraph in cell.paragraphs:
                        ...
                        if search_text in full_text_lower:
                            ...
                            if match_case:
                                ...
                                while True:
                                    # 在剩余文本中查找目标字符串
                                    pos = full_text_lower[current_pos:].find(
                                        search_text
                                    )
                                    if pos == -1:
                                        # 没有找到更多匹配，添加剩余文本
        ...
        return (
            f"成功在文档 {os.path.basename(file_path)} 中替换了 {replace_count} 处文本"
        )
...
@mcp.tool()
def delete_paragraph(file_path: str, paragraph_index: int, save: bool = True) -> str:
    """
    删除Word文档中指定的段落。
...
def insert_table_of_contents(
    file_path: str, title: str = "目录", levels: int = 3, after_paragraph: int = 0
) -> str:
    """
    在Word文档中插入目录。
    ...
    try:
        ...
        try:
            ...
            if after_paragraph >= 0 and after_paragraph < doc.Paragraphs.Count:
                ...
                toc_range.Fields.Add(
                    Range=toc_range,
                    Type=-1,
                    Text=f'TOC \\o "1-{levels}" \\h',
        ...
        try:
            ...
            for section in sections:
                ...
                if page_numbers:
                    footer = section.footer
                    footer_para = (
                        footer.paragraphs[0] if footer_text else footer.add_paragraph()
                    )
...
def set_page_layout(
    ...
    try:
        ...
        if section_index < 0 or section_index >= len(doc.sections):
            return (
                f"错误: 无效的节索引 {section_index}，文档共有 {len(doc.sections)} 节"
            )
...
@mcp.tool()
def merge_documents(main_file_path: str, files_to_merge: List[str]) -> str:
    """
    合并多个Word文档。
    ...
    try:
        ...
        try:
            ...
            return (
                f"成功将 {merged_count} 个文档合并到 {os.path.basename(main_file_path)}"
            )
