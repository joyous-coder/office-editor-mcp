        ...
        for i, slide in enumerate(prs.slides):
            ...
            for shape in slide.shapes:
                if (
                    hasattr(shape, "text")
                    and shape.text
                    and (shape.name.startswith("Title") or "标题" in shape.name)
                ):
                    slide_title = (
            ...
            slides_info.append(
                f"幻灯片 {i + 1}: {slide_title} (包含 {shape_count} 个形状)"
            )
...
def save_presentation_as(
    file_path: str, output_format: str = "pptx", new_filename: str = None
) -> str:
    """
    ...
    try:
        ...
        if output_format == "pptx":
            ...
            try:
                ...
                return (
                    f"无法完成转换: 保存为{output_format}格式需要在Windows系统上安装pywin32库和Microsoft PowerPoint。"
...
def add_slide(file_path: str, layout_name: str = "Title and Content") -> str:
    ...
    try:
        ...
        for i, name in enumerate(available_layouts):
            if (
                name.lower() == layout_name.lower()
                or layout_name.lower() in name.lower()
            ):
                layout_index = i
                break
...
def delete_slide(file_path: str, slide_index: int) -> str:
    ...
    try:
        ...
        try:
            ...
            # 如果无法替换，则返回临时文件的路径
            return (
                f"已成功创建不包含幻灯片 {slide_index} 的新演示文稿: {temp_file_path}"
                f"\n请手动替换原文件"
            )
...
def reorder_slides(file_path: str, slide_index: int, new_position: int) -> str:
    ...
    try:
        ...
        try:
            ...
            # 如果无法替换，则返回临时文件的路径
            return (
                f"已成功创建调整了幻灯片顺序的新演示文稿: {temp_file_path}"
                f"\n请手动替换原文件"
            )
...
def set_slide_background(
    file_path: str, slide_index: int, background_color: str = None
) -> str:
    """
...
def insert_shape(
    ...
    try:
        ...
        shape = slide.shapes.add_shape(
            shape_type_value, Inches(left), Inches(top), Inches(width), Inches(height)
        )
...
def add_animation(
    ...
    shape_index: int,
    animation_type: str,
    animation_trigger: str = "on_click",
