...
This module provides tools to create, edit and manage Excel workbooks.
...
def save_excel_workbook(
    file_path: str, format_type: str = "xlsx", new_filename: str = None
) -> str:
    """
    ...
    try:
        ...
        return (
            f"成功将工作簿保存为 {format_type} 格式: {os.path.basename(new_file_path)}"
        )
...
def delete_worksheet(file_path: str, sheet_name: str) -> str:
    ...
    try:
        ...
        return (
            f"成功从工作簿 {os.path.basename(file_path)} 中删除了工作表: {sheet_name}"
        )
    except Exception as e:
...
@mcp.tool()
def write_cell_range(
    file_path: str, sheet_name: str, start_cell: str, data: List[List[str]]
) -> str:
    """
    向Excel工作簿中指定区域写入数据。
    ...
    try:
        ...
        for row_idx, row_data in enumerate(data):
            for col_idx, cell_value in enumerate(row_data):
                col_letter = get_column_letter(
                    column_index_from_string(start_col_letter) + col_idx
                )
        ...
        end_col_letter = get_column_letter(
            column_index_from_string(start_col_letter) + len(data[0]) - 1
        )
...
def sort_data(
    ...
    try:
        ...
        if has_header:
            ...
            # 重新添加标题行
            sorted_df = pd.concat(
                [pd.DataFrame([headers], columns=df.columns), sorted_df]
            )
        else:
            # 直接排序所有数据
        ...
        # 获取排序依据列的字母
        sort_col_letter = get_column_letter(
            column_index_from_string(start_col_letter) + sort_column - 1
        )
...
def create_chart(
    ...
    try:
        ...
        # 创建数据引用
        data_ref = Reference(
            ws, min_col=start_col, min_row=start_row, max_col=end_col, max_row=end_row
        )
        ...
        if categories_range:
            try:
                ...
                categories = Reference(
                    ws, min_col=cats_col, min_row=cats_start_row, max_row=cats_end_row
                )
            except Exception:
...
def filter_data(
    ...
    try:
        ...
        if not ws.auto_filter:
            return (
                f"错误: 工作表 {sheet_name} 尚未应用自动筛选，请先使用apply_filter功能"
            )
        ...
        custom_filter = CustomFilter(
            operator=operator_map[filter_criteria], val=filter_value
        )
...
def create_pivot_table(
    ...
    try:
        ...
        pivot_table = PivotTable(
            name=f"PivotTable{len(wb.sheetnames)}",
            cache=pivot_cache,
...
要在Excel应用程序中查看透视表数据，请打开文件并刷新透视表。"""
...
def update_pivot_table(
    ...
    try:
        ...
        if pivot_table is None:
            return (
                f"错误: 未在工作表 {sheet_name} 中找到名为 '{pivot_name}' 的数据透视表"
            )
...
@mcp.tool()
def clear_data_validation(file_path: str, sheet_name: str, cell_range: str) -> str:
    """
    ...
    try:
        ...
        for dv in ws.data_validations.dataValidation:
            ...
            for dv_range in dv.sqref.ranges:
                dv_min_col, dv_min_row, dv_max_col, dv_max_row = range_boundaries(
                    str(dv_range)
                )
                ...
                if (
                    dv_min_col <= max_col
                    and dv_max_col >= min_col
                    and dv_min_row <= max_row
                    and dv_max_row >= min_row
                ):
...
def add_conditional_formatting(
    ...
    try:
        ...
        if color or text_color or bold or italic or underline:
            ...
            if color:
                try:
                    ...
                    fill = PatternFill(
                        start_color=color, end_color=color, patternType=fill_type
                    )
        ...
        if condition_type == "cellIs":
            ...
            rule = FormulaRule(
                formula=[condition_value], stopIfTrue=False, dxf=diff_style
            )
            ...
            rule = Rule(type="aboveAverage", aboveAverage=above, dxf=diff_style)

        elif condition_type in ["duplicateValues", "uniqueValues"]:
            # 创建重复值或唯一值规则
            rule = Rule(type=condition_type, dxf=diff_style)
            ...
            rule = FormulaRule(formula=[formula], stopIfTrue=False, dxf=diff_style)
...
def add_data_bar(
    ...
    if min_type not in valid_types:
        return (
            f"错误: 无效的最小值类型 '{min_type}'，可选值为: {', '.join(valid_types)}"
        )
    if max_type not in valid_types:
        return (
            f"错误: 无效的最大值类型 '{max_type}'，可选值为: {', '.join(valid_types)}"
        )
...
def add_color_scale(
    ...
    if mid_type is not None and mid_type not in valid_types:
        return (
            f"错误: 无效的中间值类型 '{mid_type}'，可选值为: {', '.join(valid_types)}"
        )
    if max_type not in valid_types:
        return (
            f"错误: 无效的最大值类型 '{max_type}'，可选值为: {', '.join(valid_types)}"
...
def clear_conditional_formatting(
    file_path: str, sheet_name: str, cell_range: str = None
) -> str:
    """
    清除Excel工作簿中的条件格式。
        ...
        if cell_range:
            ...
            for cf in ws.conditional_formatting:
                for range_str in cf.sqref.ranges:
                    cf_min_col, cf_min_row, cf_max_col, cf_max_row = range_boundaries(
                        str(range_str)
                    )
                    ...
                    if (
                        cf_min_col <= max_col
                        and cf_max_col >= min_col
                        and cf_min_row <= max_row
                        and cf_max_row >= min_row
                    ):
                    ...
                    if match_entire_cell:
                        ...
                        if match_case:
                            ...
                            if find_text in original_value:
                                cell.value = original_value.replace(
...
@mcp.tool()
def apply_sum(
    file_path: str, sheet_name: str, target_cell: str, range_to_sum: str
) -> str:
    """
    在Excel工作簿中应用SUM函数。
...
@mcp.tool()
def apply_average(
    file_path: str, sheet_name: str, target_cell: str, range_to_average: str
) -> str:
    """
...
@mcp.tool()
def apply_count(
    file_path: str, sheet_name: str, target_cell: str, range_to_count: str
) -> str:
    """
    在Excel工作簿中应用COUNT函数。
...
@mcp.tool()
def apply_max(
    file_path: str, sheet_name: str, target_cell: str, range_to_max: str
) -> str:
    """
    在Excel工作簿中应用MAX函数。
...
@mcp.tool()
def apply_min(
    file_path: str, sheet_name: str, target_cell: str, range_to_min: str
) -> str:
    """
    在Excel工作簿中应用MIN函数。
...
def apply_countif(
    ...
    try:
        # 如果条件是文本且未添加引号，则添加引号
        if not (criteria.startswith('"') and criteria.endswith('"')) and not any(
            op in criteria for op in [">", "<", "="]
        ):
            criteria = f'"{criteria}"'
...
def apply_vlookup(
    ...
    try:
        ...
        if (
            not re.match(r"^[A-Za-z]+[0-9]+$", lookup_value)
            and not lookup_value.isdigit()
        ):
...
def apply_if(
    ...
    try:
        ...
        for value in [value_if_true, value_if_false]:
            if (
                not re.match(r"^[A-Za-z]+[0-9]+$", value)
